import SpaceBalancer from "./[x]SpaceBalancer.js";
...
describe("50명 뷰어 생성 후 50명 로그인", () => {
  ...
  test("50명 플레이어 생성", () => {
    const viewers1 = new Array(50).fill(0).map((u, i) => {
      const user = new User({
        id: i,
        type: "viewer",
        nickname: "John",
    ...
    const viewers2 = new Array(50).fill(0).map((u, i) => {
      const user = {
        deviceID: i,
      };
    ...
    const players = new Array(50).fill(0).map((u, i) => {
      const user = new User({
        id: i,
        type: "player",
        nickname: "John",
    ...
    console.log("space size", sb.spaceSize("A"));
    console.log("space channel", sb.channelSize("A", 1));
    console.log(sb.checkChannelUserAmountByType("A", 1, "viewer"));
...
describe("새로운 함수 테스트", () => {
  ...
  test("selectUserByDeviceID 테스트 deviceID 없을 때", () => {
    expect(sb.selectUserByDeviceID(53)).toEqual({});
  });
  test("selectUserByDeviceID 테스트 deviceID 있을 때", () => {
    console.log(sb.selectChannel('A', 1))
    expect(sb.selectUserByDeviceID(2)).toStrictEqual({
      id: 2,
      type: "viewer",
      nickname: "John",
      deviceID: 2,
